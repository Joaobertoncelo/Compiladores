...
public class AnaliseSemantica {
    ...
    private Symbol funcaoAtual; // Nova variável para armazenar a função atual

    public AnaliseSemantica() {
        tabelaSimbolos = new SymbolTable();
        contextStack = new Stack<>();
        inicializarFuncoesEmbutidas(); // Inicializa funções embutidas
    }

    /**
     * Inicializa funções embutidas na tabela de símbolos.
     */
    private void inicializarFuncoesEmbutidas() {
        try {
            // Em vez de ter esta lista fixa de 5 parâmetros para console:
            //   List<TipoSemantico> parametrosConsole = Arrays.asList(
            ...
            //       TipoSemantico.CHAR
            //   );
            //   tabelaSimbolos.adicionarSimbolo("console", TipoSemantico.VOID, parametrosConsole);

            // Substitua por VARARGS (String, VARARGS):
            ...
            // Exemplo de overwatch sem mudanças
            List<TipoSemantico> parametrosOverwatch1 = Arrays.asList(TipoSemantico.STRING);
            ...
            // Resto das funções embutidas
            List<TipoSemantico> parametrosBaiter = Arrays.asList(TipoSemantico.INT);
            tabelaSimbolos.adicionarSimbolo("baiter", TipoSemantico.VOID, parametrosBaiter);
    ...
    private void visitar(NodoAST nodo) throws SemanticException, SyntaxException {
        switch (nodo.getTipo()) {
            case PROGRAMA:
                // Definir a função principal 'main' com tipo de retorno INT
                List<TipoSemantico> parametrosMain = new ArrayList<>(); // Função main sem parâmetros
                tabelaSimbolos.adicionarSimbolo("main", TipoSemantico.INT, parametrosMain);

                // Obter o símbolo da função main
                ...
                funcaoAtual = simbolosMain.get(0); // define a função atual como main

                contextStack.push(ContextType.FUNCTION);
                tabelaSimbolos.entrarEscopo(); // Entrar no escopo da função main

                visitarBloco(nodo); // Visitar o bloco principal

                tabelaSimbolos.sairEscopo(); // Sair do escopo da função main
                contextStack.pop(); // Desempilhar o contexto de função

                funcaoAtual = null; // Ao sair do main, limpa a função atual
                break;
            case BLOCO:
                visitarBloco(nodo);
                break;
    ...
    private void visitarDeclaracaoVariavel(NodoAST decl) throws SemanticException, SyntaxException {
        ...
        if (decl.getFilhos().size() > 1) {
            ...
            if (!tipoVar.isCompatible(tipoExpr)) {
                throw new SemanticException("Erro semântico: Tipo da expressão '" + tipoExpr + "' não compatível com o tipo da variável '" + tipoVar + "'.");
            }
        }
    }
    ...
    private void visitarAtribuicao(NodoAST atrib) throws SemanticException, SyntaxException {
        ...
        if (!simbolo.getTipo().isCompatible(tipoExpr)) {
            throw new SemanticException("Erro semântico: Tipo da expressão '" + tipoExpr + "' não compatível com o tipo da variável '" + simbolo.getTipo() + "'.");
        }
    }
    ...
    private TipoSemantico visitarExpressao(NodoAST expr) throws SemanticException, SyntaxException {
        switch (expr.getTipo()) {
            ...
            default:
                throw new SemanticException("Erro semântico: Expressão desconhecida.");
        }
    }
    ...
    private TipoSemantico analisarExpressaoBinaria(NodoAST expr) throws SemanticException, SyntaxException {
        ...
        if (op.equals("++") || op.equals("--") || op.equals("!")) {
            if (expr.getFilhos().size() < 1) {
                throw new SemanticException("Erro inesperado: Operador unário '" + op + "' com número de operandos incorreto.");
            }
            ...
            switch (op) {
                ...
                default:
                    throw new SemanticException("Erro semântico: Operador desconhecido '" + op + "'.");
            }
        } else {
            // Operadores binários
            if (expr.getFilhos().size() < 2) {
                throw new SemanticException("Erro inesperado: Operador binário '" + op + "' com número de operandos incorreto.");
            }
    ...
    private TipoSemantico visitarFuncCall(NodoAST funcCallNode) throws SemanticException, SyntaxException {
        ...
        if (simbolosFunc == null || simbolosFunc.isEmpty()) {
            throw new SemanticException("Erro semântico: Função '" + nomeFuncao + "' não declarada.");
        }

        List<NodoAST> argumentos = funcCallNode.getFilhos();
        ...
        throw new SemanticException("Erro semântico: Função '" + nomeFuncao + "' chamada com " + tiposArgumentos.size() + " argumentos, mas nenhuma assinatura corresponde.");
    }
    ...
    private void visitarFor(NodoAST forNode) throws SemanticException, SyntaxException {
        // Estrutura esperada: [decl, cond, incremento, bloco]
    ...
    private void visitarReturn(NodoAST returnNode) throws SemanticException, SyntaxException {
        System.out.println("Analisando 'return' dentro do contexto: " + contextStack);
        ...
        if (!funcaoAtual.getTipo().isCompatible(tipoExpr)) {
            throw new SemanticException("Erro semântico: Tipo do 'return' (" + tipoExpr + ") não compatível com o tipo da função (" + funcaoAtual.getTipo() + ").");
        }
    }

    private void visitarBreak(NodoAST breakNode) throws SemanticException, SyntaxException {
        if (contextStack.isEmpty() || !(contextStack.peek() == ContextType.LOOP || contextStack.peek() == ContextType.SWITCH)) {
    ...
    private void visitarFuncDefinicao(NodoAST funcDefNode) throws SemanticException, SyntaxException {
        ...
        for (Symbol simboloFunc : simbolosFunc) {
            if (simboloFunc.isFunction() && simboloFunc.getTipo().equals(tipoReturn) && simboloFunc.getParametros().equals(tiposParametros)) {
                throw new SemanticException("Erro semântico: Função '" + nomeFunc + "' já foi definida com esta assinatura.");
            }
        }

        tabelaSimbolos.adicionarSimbolo(nomeFunc, tipoReturn, tiposParametros);

        // Obter a referência da função atual
        ...
        if (funcSymbol == null) {
            throw new SemanticException("Erro semântico: Não foi possível recuperar a função definida '" + nomeFunc + "'.");
        }

        funcaoAtual = funcSymbol; // define a função atual
        ...
        // Ao sair da função, limpa a função atual
        funcaoAtual = null;
    }
    ...
    private List<TipoSemantico> visitarParametros(NodoAST paramsNodo) throws SemanticException, SyntaxException {
        ...
        for (NodoAST param : paramsNodo.getFilhos()) {
            String tipoStr = param.getFilhos().get(0).getValor(); // tipo
    ...
    private TipoSemantico verificarTipoAritmetico(TipoSemantico esquerda, TipoSemantico direita) throws SemanticException {
        if (esquerda == TipoSemantico.INT && direita == TipoSemantico.INT) {
            ...
            return TipoSemantico.FLOAT;
        } else if ((esquerda == TipoSemantico.INT || esquerda == TipoSemantico.FLOAT || esquerda == TipoSemantico.DOUBLE) &&
                   (direita == TipoSemantico.INT || direita == TipoSemantico.FLOAT || direita == TipoSemantico.DOUBLE)) {
            return TipoSemantico.DOUBLE;
        } else {
            throw new SemanticException("Erro semântico: Tipos incompatíveis para operação aritmética: " + esquerda + " e " + direita + ".");
        }
    }

    private void verificarCompatibilidadeComparacao(TipoSemantico esquerda, TipoSemantico direita) throws SemanticException {
        if (!esquerda.isCompatibleForComparison(direita)) {
            throw new SemanticException("Erro semântico: Operador de comparação usado com tipos incompatíveis: " + esquerda + " e " + direita + ".");
        }
    }
    ...
    private void verificarTipoBooleano(TipoSemantico esquerda, TipoSemantico direita) throws SemanticException {
        if (esquerda != TipoSemantico.BOOLEAN || direita != TipoSemantico.BOOLEAN) {
            throw new SemanticException("Erro semântico: Operadores lógicos '&&' e '||' requerem operandos do tipo BOOLEAN.");
        }
    }
    ...
    private void verificarTipoIncremento(TipoSemantico tipo) throws SemanticException {
        if (!(tipo == TipoSemantico.INT || tipo == TipoSemantico.FLOAT || tipo == TipoSemantico.DOUBLE)) {
            throw new SemanticException("Erro semântico: Operador de incremento '++' ou '--' requer operando de tipo INT, FLOAT ou DOUBLE.");
        }
    }
    ...
    private boolean isFloat(String valor) {
        try {
            ...
            return valor.contains(".") && !valor.contains("f"); // Ajuste simples, dependendo da linguagem
        } catch (NumberFormatException e) {
            return false;
        }
    }
    ...
    private boolean isChar(String valor) {
        return valor.length() == 3 && valor.startsWith("'") && valor.endsWith("'"); // e.g., 'a'
    }
        ...
        for (int i = escopos.size() - 1; i >= 0; i--) {
            ...
            for (Map.Entry<String, List<Symbol>> entry : escopo.entrySet()) {
                ...
                for (Symbol simbolo : simbolos) {
                    if (simbolo.isFunction()) {
                        System.out.println("  Função: " + simbolo.getNome() + " -> " + simbolo.getTipo() + " com parâmetros " + simbolo.getParametros());
                    } else {
